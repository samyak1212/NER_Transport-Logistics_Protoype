  ...
  return (
    <div className="space-y-4 text-slate-200 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <form onSubmit={handleCalculate} className="mt-3 space-y-3 text-xs">
            ...
            <div>
              ...
              >
                ...
                <option value="FUEL_POL">Petroleum, Oil & Lubricants (&lambda;=1.8 High Hazard)</option>
                <option value="CONSTRUCTION">Construction Materials (&lambda;=0.8 Cost/Time)</option>
                <option value="GENERAL">General Commercial Freight (&lambda;=1.0 Standard)</option>
              </select>
            </div>
            ...
            <button
              ...
              className="w-full mt-2 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold rounded-lg shadow-lg shadow-cyan-500/20 flex items-center justify-center gap-2 transition-all disabled:opacity-50 cursor-pointer"
            >
              {isLoadingRoute ? (
        ...
        <div className="lg:col-span-2 glass-panel p-4 rounded-xl border border-slate-800 shadow-lg flex flex-col justify-between">
          <div>
              ...
              <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="p-3 rounded-lg bg-defense-900 border border-slate-800 text-xs">
                  <div className="flex justify-between items-center text-slate-400 pb-1 border-b border-slate-800">
                    ...
                    <span className="font-mono text-slate-500">&lambda; = 0.0</span>
                  </div>
          ...
          {comparisonData && (
            <div className="mt-3 p-2.5 rounded-lg bg-cyan-950/40 border border-cyan-800/60 text-cyan-200 text-xs flex items-start gap-2 font-mono">
      ...
      {/* 3. Focused Mission Navigation Highway Route Preview (Clean Google Maps View) */}
      <div className="glass-panel p-4 rounded-xl border border-slate-800 shadow-xl space-y-3">
        <div className="flex justify-between items-center pb-2 border-b border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-xs uppercase tracking-wider text-slate-200">
              Mission Navigation Highway Geometry (Google Driving Directions)
            </h3>
          </div>
        ...
        <div className="relative w-full h-[380px] rounded-xl overflow-hidden border border-slate-800">
          <MapCanvas
            nodes={nodes}
            segments={segments}
            ...
            selectedCorridor="CORRIDOR_NH13"
          />
        </div>
      </div>

      {/* 4. Live Monitored Convoy Telemetry & Controls */}
      <div className="glass-panel p-4 rounded-xl border border-slate-800 shadow-xl">
        <div className="flex flex-wrap items-center justify-between gap-2 pb-2.5 border-b border-slate-800">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 bg-cyan-500 hover:bg-cyan-600 text-white rounded-lg text-xs font-bold flex items-center gap-1.5 shadow-md shadow-cyan-500/20 transition-all cursor-pointer"
            >
            ...
            {activeVehicle?.status === 'PAUSED' ? (
              <button
                ...
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-emerald-400 rounded-lg text-xs font-bold transition-all cursor-pointer"
              >
                Resume
              </button>
            ) : (
              <button
                ...
                className="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-amber-400 rounded-lg text-xs font-bold transition-all cursor-pointer"
              >
                Pause
              </button>
            )}
        ...
        <div className="mt-3 grid grid-cols-2 md:grid-cols-5 gap-3 font-mono text-xs">
          ...
          <div className="p-2.5 rounded-lg bg-defense-900 border border-slate-800">
            ...
            <div className="text-sm font-bold text-emerald-400 mt-0.5">{(activeVehicle?.progress_pct || 0).toFixed(1)}%</div>
            <div className="text-[10px] text-slate-500">
              {(activeVehicle?.distance_covered_km || 0).toFixed(1)} / {activeVehicle?.total_distance_km || 380} km
            </div>
          </div>
